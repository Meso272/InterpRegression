	...
	for(i=offset;i<n3-offset;i+=2)
		for(j = offset;j<n2-offset;j+=2)
			for(k =offset;k<n1-offset;k+=2)
			{
				...
				float predValue = coefficients[0]*rawData[index-1] + coefficients[1]*rawData[index+1] + coefficients[2]*rawData[index-n1] + coefficients[3]*rawData[index+n1]
...
int main(int argc, char*argv[])
{
	...
	int offset = 1;
	size_t nbPoints = 0;
	double* coefficients = calculateCoefficients_3D_X3D6Points(6, n3, n2, n1, offset, rawData, &nbPoints);
