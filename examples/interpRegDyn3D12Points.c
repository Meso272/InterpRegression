	...
	for(i=offset;i<n3-offset;i+=2)
		for(j = offset;j<n2-offset;j+=2)
			for(k =offset;k<n1-offset;k+=2)
			{
				...
				float predValue = coefficients[0]*rawData[index-1] + coefficients[1]*rawData[index+1] 
				+ coefficients[2]*rawData[index-n1] + coefficients[3]*rawData[index+n1]
				+ coefficients[4]*rawData[index-n2xn1] + coefficients[5]*rawData[index+n2xn1] 
				+ coefficients[6]*rawData[index-3] + coefficients[7]*rawData[index+3] 
				+ coefficients[8]*rawData[index-n1x3] + coefficients[9]*rawData[index+n1x3] 
				+ coefficients[10]*rawData[index-n2xn1x3] + coefficients[11]*rawData[index+n2xn1x3] ;																
...
int main(int argc, char*argv[])
{
	...
	int offset = 3;
	size_t nbPoints = 0;
	double* coefficients = calculateCoefficients_3D_X3D12Points(12, n3, n2, n1, offset, rawData, &nbPoints);
	printf("coefficients = %f %f %f %f %f %f, nbPoints = %zu\n", coefficients[0], coefficients[1], coefficients[2], coefficients[3], coefficients[4], coefficients[5], nbPoints);
